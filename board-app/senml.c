    ...
    for (size_t i = 0, j = num_toks; j > 0; i++, j--) {
        ...
        if (last_type == JSMN_OBJECT && t->type == JSMN_STRING) {
            ...
            if (!strcmp(tmp_key, SJ_NAME)) {
                ...
                (pack->records)[array_num].value_type = SENML_TYPE_INT;
...
int senml_encode_json_s(const senml_pack_t *pack, char *output, size_t len)
{
    ...
    if (pack->base_info) {
        ...
        if (pack->base_info->version) {
            chars_written = snprintf(&output[ins_pos], buf_len, "\"bver\":%d,",
        ...
        if (pack->base_info->base_time != 0) {
            chars_written = snprintf(&output[ins_pos], buf_len, "\"bt\":%lu,",
        ...
        if (pack->base_info->base_value != 0) {
            chars_written = snprintf(&output[ins_pos], buf_len, "\"bv\":%lu,",
        ...
        if (pack->base_info->base_sum != 0) {
            chars_written = snprintf(&output[ins_pos], buf_len, "\"bs\":%lu,",
    ...
    for (size_t i = 0; i < pack->num; i++) {
        ...
        if (curr_record->time != 0) {
            chars_written = snprintf(&output[ins_pos], buf_len, "\"t\":%ld,",
                                     curr_record->time);
        ...
        if (curr_record->update_time != 0) {
            chars_written = snprintf(&output[ins_pos], buf_len, "\"ut\":%ld,",
        ...
        if (curr_record->value_sum != 0) {
            chars_written = snprintf(&output[ins_pos], buf_len, "\"s\":%lu,",
        ...
        if (curr_record->value_type != SENML_TYPE_UNDEF) {
            if (curr_record->value_type == SENML_TYPE_INT) {
                chars_written = snprintf(&output[ins_pos], buf_len, "\"v\":%lu,",
                                         curr_record->value.f);
            }
